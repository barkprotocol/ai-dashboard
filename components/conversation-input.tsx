...
import { ImageIcon, Link, SendHorizontal, X } from "lucide-react"
...
function LinkPreview({ link, onRemove }: { link: InsertedLink; onRemove: () => void }) {
  return (
    <div className="flex items-center justify-between rounded-md bg-muted p-2">
      ...
      <button
        type="button"
        onClick={onRemove}
        className="rounded-full bg-background/80 p-1 shadow-sm backdrop-blur-sm transition-all hover:bg-background"
      >
        <X className="h-3 w-3" />
      </button>
    </div>
...
export function ConversationInput({
  ...
  const handleImageUpload = useCallback(async (files: File[]) => {
    const uploadPromises = files.map(async (file) => {
      ...
      try {
        ...
        setAttachments((prev) =>
          prev.map((att) => (att.localUrl === localUrl ? { ...att, url, uploading: false } : att)),
        )
      } catch (error) {
  ...
  const handleLinkInsert = useCallback(() => {
    ...
    const title = prompt("Enter a title for the link:") || url
    setInsertedLinks((prev) => [...prev, { url, title }])
  }, [])

  const removeLink = useCallback((url: string) => {
    setInsertedLinks((prev) => prev.filter((link) => link.url !== url))
  }, [])

  const handleSubmit = async (e?: React.FormEvent) => {
    e?.preventDefault()
    if (!value.trim() && attachments.length === 0 && insertedLinks.length === 0) return
    ...
    <div className={`relative ${!onChat ? "duration-500 animate-in fade-in slide-in-from-bottom-4" : ""}`}>
      <div className="relative rounded-xl bg-muted">
        <form onSubmit={handleSubmit} className="flex flex-col">
          ...
          {(attachments.length > 0 || insertedLinks.length > 0) && (
            <div className="flex flex-wrap gap-2 rounded-t-xl bg-muted/50 p-3">
          ...
          <Textarea
            ...
            className={cn(
              ...
              attachments.length > 0 || insertedLinks.length > 0 ? "rounded-t-none" : "rounded-t-xl",
            )}
          />
          ...
          <div className="flex items-center justify-between border-t px-4 py-2">
            ...
            <div className="flex">
              ...
              <Button
                ...
                variant="ghost"
                disabled={
                  (!value.trim() && attachments.length === 0 && insertedLinks.length === 0) ||
