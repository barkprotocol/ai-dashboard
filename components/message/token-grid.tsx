    ...
    >
      ...
      <div className="flex items-center gap-3 p-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className={cn("mt-1 text-xs font-medium", change >= 0 ? "text-green-500" : "text-red-500")}>
            {change >= 0 ? "+" : ""}
            {change.toFixed(2)}%
          </div>
        </div>
...
export function TokenGrid({ tokens, className, isLoading = false, timeframe = "24h" }: TokenGridProps) {
  if (isLoading) {
    return (
  ...
  return (
    <div
      className={cn(
        "grid gap-4",
        tokens.length === 1 ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3",
        className,
      )}
    >
      {tokens.map((token) => (
