      ...
      <div className="relative flex items-center justify-between p-3 sm:p-4">
        ...
        <div className="flex min-w-0 items-center gap-4">
          <div className="relative">
            <Avatar className="h-8 w-8 rounded-lg border bg-gradient-to-br from-background to-muted sm:h-10 sm:w-10 sm:rounded-xl">
              <AvatarImage
                src={token.imageUrl}
                alt={token.name}
          ...
          <div className="min-w-0">
            ...
            <div className="mt-1.5 flex items-baseline gap-2 text-xs sm:text-sm">
              <span className="font-medium text-muted-foreground">
                {formatNumber(token.balance, "number", 4)} {token.symbol}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-end gap-1.5">
          ...
          {hasPrice && (
            <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground sm:text-xs">
              <span>@ {formatNumber(token.pricePerToken, "currency", 4)}</span>
...
export function WalletPortfolio({ data, className, isLoading = false }: WalletPortfolioProps) {
  if (isLoading) {
    return (
      <Card className={cn("overflow-hidden border-border/50 bg-gradient-to-br from-background to-muted/30", className)}>
  ...
  return (
    <Card className={cn("overflow-hidden border-border/50 bg-gradient-to-br from-background to-muted/30", className)}>
      <CardHeader className="border-b border-border/40 bg-muted/20 p-3">
        <CardTitle className="flex items-center justify-between text-sm">
          ...
          <span className="text-sm font-medium">{formatNumber(data.totalBalance, "currency")}</span>
        </CardTitle>
      </CardHeader>
      <CardContent className="p-0">
        <ScrollArea className="h-[300px]">
