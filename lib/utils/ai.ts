...
import type { ToolUpdate } from "@/types/util"

export function getUnconfirmedConfirmationMessage(messages: Array<Message>): Message | undefined {
  const unconfirmedConfirmationMessage = messages.find(
    (msg) =>
      msg.role === "assistant" &&
      msg.toolInvocations?.find(
        (tool) => tool.toolName === "askForConfirmation" && tool.state === "call" && !(tool as any).result,
      ),
  )
 ...
 */
export function updateConfirmationMessageResult(confirmationMessage: CoreToolMessage, isConfirmed: boolean) {
 ...
 */
export function getToolMessageResult(message: CoreToolMessage): ToolMessageResult | undefined {
  const content = message.content?.at(0)

  return content && content.result ? (content.result as ToolMessageResult) : undefined
}
 ...
 */
export function getMostRecentToolResultMessage(messages: Array<CoreMessage>): CoreToolMessage | undefined {
