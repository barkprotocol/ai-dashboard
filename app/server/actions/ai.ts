  ...
  .action(async ({ parsedInput: { id, title } }): Promise<ActionEmptyResponse> => {
    try {
      ...
      if (mockConversations.has(id)) {
        mockConversations.set(id, { ...mockConversations.get(id), title })
      }
      return { success: true }
    } catch (error) {
...
export const getAgentKit = async ({
  ...
  const wallet = { publicKey: "mock_public_key", encryptedPrivateKey: "mock_encrypted_private_key" }
  ...
  if (wallet.encryptedPrivateKey) {
    ...
    // For Privy wallets, we need to handle this differently
    // This is a placeholder and might need to be adjusted based on how Privy handles private keys
    privateKey = "mock_privy_private_key"
  }
