...
import { BaseWallet, SolanaAgentKit, type WalletAdapter } from "solana-agent-kit"
import { z } from "zod"
...
import { getPrivyClient, verifyUser } from "../user/route"
...
async function convertUserResponseToBoolean(message: string) {
...
async function getAgentKit({
  ...
  if (wallet.encryptedPrivateKey) {
    walletAdapter = new BaseWallet(await decryptPrivateKey(wallet?.encryptedPrivateKey))
  } else {
  ...
  if (!openaiKey) {
    return { success: false, error: "OPENAI_API_KEY_NOT_FOUND" }
  }

  const heliusApiKey = process.env.HELIUS_API_KEY
  if (!heliusApiKey) {
    return { success: false, error: "HELIUS_API_KEY_NOT_FOUND" }
  }
