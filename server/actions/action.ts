...
import { retrieveAgentKitServer } from "@/lib/utils"
...
export async function processAction(action: Action) {
  ...
  try {
    ...
    // Get user wallet
    const activeWallet = action.user.wallets.find((w: { active: any }) => w.active)
    if (!activeWallet) {
    ...
    if (messages && isValidTokenUsage(usage)) {
      const messageIds = messages.map((message: { id: any }) => message.id)
